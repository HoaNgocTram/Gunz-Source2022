...
bool MBMatchServerShopXmlReloadObj::OnReload()
{
	MMatchShop::GetInstance()->Clear();
	MMatchShop::GetInstance()->Create("shop.xml");
	MMatchShop::GetInstance()->Create("shopcash.xml");
	MMatchShop::GetInstance()->Create("shopmedals.xml");
	MMatchShop::GetInstance()->Create("shopevent.xml");

	mlog( "MBMatchServerShopXmlReloadObj::OnReload - success reload %s\n",
		GetFileName().c_str() );
...
// Custom: Reload object map
bool MBMatchServerConfigReloader::ReloadObjMapRun()
{
	if( m_ReloadObjMap.empty() )
		return false;
	
	// LUL REFACTORING
	for( ReloadObjMap::iterator it = m_ReloadObjMap.begin(); it != m_ReloadObjMap.end(); ++it )
	{
		if ((*it).first != "shop.xml")
			(*it).second->Reload();
	}
...
bool MBMatchServerConfigReloader::InitReloadObjMap()
{
	...
	if( !InsertReloadMap(string("shop.xml"), new MBMatchServerShopXmlReloadObj) )
		return false;

	if (!InsertReloadMap(string("shopcash.xml"), new MBMatchServerShopXmlReloadObj))
		return false;

	if (!InsertReloadMap(string("shopmedals.xml"), new MBMatchServerShopXmlReloadObj))
		return false;

	if (!InsertReloadMap(string("shopevent.xml"), new MBMatchServerShopXmlReloadObj))
		return false;

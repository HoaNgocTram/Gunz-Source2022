...
const bool MBMatchServer::CheckUserCanBuyItem( MMatchObject* pObj, const int nItemID, const int nItemCnt, const DWORD dwPrice, MSHOPMODE nShopMode )
{
	...
	{
		//LOG(LOG_PROG, "passing by if (nShopMode == SHOP_CASH)");
	...
	else if (nShopMode == SHOP_MEDAL)// 3
	{
		LOG(LOG_PROG, "passing by if (nShopMode == SHOP_MEDAL)");
	...
	{
		LOG(LOG_PROG, "passing by if (nShopMode == SHOP_EVENT)");

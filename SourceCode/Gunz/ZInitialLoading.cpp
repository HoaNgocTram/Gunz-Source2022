...
#ifndef _HIDETHINGLOADING
			sprintf(buffer, "Loading %s ...", m_strLoadingStr.c_str());
#else
			sprintf(buffer, "Loading files ...", m_strLoadingStr.c_str());
#endif
...
void	ZInitialLoading::Draw(LOADING_SCENE_MODE mode_ /* = MODE_DEFAULT */, int destIndex_ /* = -1 */, bool bFlip_, bool fromThread_)
{
	...
	float fGradeBlendFacter = 1.f;
	if (!m_bGradeFadeOutStart && timeGetTime() - m_Grade_Time > GRADE_TIMER - 2)	// 3ﾃﾊ ｵｿｾﾈｸｸ ｺｸｿｩﾁﾜ
	{	// 3ﾃﾊﾈﾄｿ｡ 15ｼｼ ｵ鉷ﾞ ﾀﾌｹﾌﾁ・ﾆ菎ﾌｵ・ｾﾆｿｳｸｮ ｽﾃﾀﾛﾇﾘ ﾁﾜ
		numDrawing = FADEOUT_NUM_DRAWING;
		fGradeBlendFacter = 1.f;
		m_bGradeFadeOutStart = true;
	}
...
/*		if(m_pLoadingStr && mpDC ) {
		...
		// ﾃｻｼﾒｳ・ﾀﾚﾀｲ ｱﾔﾁｦ ﾀ釤・ﾈ(ｾｲｹ悊ｲｹ悊ｲｹ・..)...
#ifdef LOCALE_KOREA
		if (mpDC)
		{
...
void	ZInitialLoading::Release()
{
	if (!mbUseEnable) return;	// ﾈｰｼｺﾈｭ ｵﾇｾ・ﾀﾖﾁ・ｾﾊﾀｸｸ・skip
...
void	ZInitialLoading::DrawTips()
{
	...
	if ((g_pDC != 0) && (pFont != 0))
	{
		...
		else
		{
			...
			// ｷﾎｵ・ﾈｭｸ鯀｡ ｸﾂﾃ郛ｭ ﾅﾘｽｺﾆｮｸｦ ｼｳﾁ､ﾇﾑｴﾙ.(ﾇﾏｵ蠧ﾚｵ・.. -_-;)
			int nPosx, nPosy;
			char szStr1[256], szStr2[256], szStr3[256];
			switch (m_nTipNum)
			{

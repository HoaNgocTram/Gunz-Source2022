...
void VoiceChat::StartRecording()
{
	/*if (ZGetGame()->GetMatch()->IsTeamPlay() || 
		...
		ZGetGameClient()->IsCWChannel() || ZGetGameClient()->IsLadderChannel())*/
		...
		if (ZGetGame() && ZGetConfiguration()->GetEtc()->bVoiceChat)
		{
			...
			/*ZCharacter* pChar = (ZCharacter*)ZGetCharacterManager();
			if (!pChar->IsVisible() || pChar->IsDie()) return;*/
...
void VoiceChat::OnReceiveVoiceChat(ZCharacter* Char, const uint8_t* Buffer, int Length)
{
	/*if (ZGetGame()->GetMatch()->IsTeamPlay() ||
		...
		if (ZGetGame() && ZGetConfiguration()->GetEtc()->bVoiceChat)
		{
			/*if (Z_AUDIO_VOICE_VOLUME / 10000.f)
			{*/
				if (!CanDecode)
					return;
	...
	void* userData)
{
	/*if (ZGetGame()->GetMatch()->IsTeamPlay() ||
		...
		if (ZGetGame() && ZGetConfiguration()->GetEtc()->bVoiceChat)
		{
			/*if (ZGetGame() && ZGetConfiguration()->GetAudio()->bVoiceChatVolumen)
			{*/
...
{
	auto Success = SpeakerBitmap.Create("SpeakerIcon", RGetDevice(), "Interface/default/png/SpeakerIcon.png");
	if (!Success)

...
BEGIN_IMPLEMENT_LISTENER(ZGetBattleExitButtonListener, MBTN_CLK_MSG)
	// 클랜전이 진행중이면 종료하지 않는다
	if (!ZGetGameClient()->IsCWChannel() && !ZGetGameClient()->IsLadderChannel())
	{
		if(pWidget->GetParent()!=NULL) pWidget->GetParent()->Show(false);
...
/*
	...
	MButton* pButton = (MButton*)pResource->FindWidget("ShopCaller");
	if (pButton)
	{
	...
	ZGetShop()->m_ShopMode = ZSHOP_MEDAL;
...
BEGIN_IMPLEMENT_LISTENER(ZGetCreateCharacterButtonListener, MBTN_CLK_MSG)
	...
	if(nEmptySlotIndex>=0)
	{
		...
		// Custom: NameChar Off Filter (Only name KOR)
...
END_IMPLEMENT_LISTENER()
BEGIN_IMPLEMENT_LISTENER(ZGetEventEquipmentListCallerButtonListener, MBTN_CLK_MSG)
ZGetGameInterface()->GetShopEquipInterface()->SelectShopTab(3);
END_IMPLEMENT_LISTENER()
...
ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
MWidget* pWidget = pResource->FindWidget("LadderWarsMapVote");
...
if (pWidget != NULL)
{
	...
	for (int i = 0; i < 3; i++)
	{
		sprintf(Name, "LadderWarsVote%d", i);
...
MLabel* pLabel = (MLabel*)pResource->FindWidget("LadderWarsVote0");
if (pLabel)
pLabel->SetTextColor(MCOLOR(102, 205, 0));
ZPostLadderWarVote(0);
END_IMPLEMENT_LISTENER();
...
MLabel* pLabel = (MLabel*)pResource->FindWidget("LadderWarsVote1");
if (pLabel)
pLabel->SetTextColor(MCOLOR(102, 205, 0));
ZPostLadderWarVote(1);
END_IMPLEMENT_LISTENER();
...
MLabel* pLabel = (MLabel*)pResource->FindWidget("LadderWarsVote2");
if (pLabel)
pLabel->SetTextColor(MCOLOR(102, 205, 0));
ZPostLadderWarVote(2);
END_IMPLEMENT_LISTENER();
#endif // _CW_VOTE
		...
		case MSM_CLAN:
			{
				...
				else if (nCount == 0)
				{
					...
					// Balance 옵션
					int nBalancedMatching = 0, GameType = 0;
					ZIDLResource* pResource = ZGetGameInterface()->GetIDLResource();
					...
					ZPostLadderRequestChallenge(ppMember, 1, nBalancedMatching, GameType);
				}
#endif
				else
				{
...
if (pWidget != NULL)
pWidget->Show(false);
bool bLadderWarUI = (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_LADDER);
ZPostLadderCancel(bLadderWarUI);
END_IMPLEMENT_LISTENER();

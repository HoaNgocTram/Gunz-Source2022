	...
	case MC_SPY_GAME_INFO	:
		{
			if(ZGetGame()->GetMatch()->GetRoundState() == MMATCH_ROUNDSTATE_PLAY)
				if(!m_vtLastSpyItem.empty() || !m_vtLastTrackerItem.empty())
					break;
			...
			{
				ZCharacter* pCurrChar = (ZCharacter*)(*i).second;
			...
			for(int i = 0; i < nSpyCount; i++)
			{
				...
				ZCharacter* pCharacter = (ZCharacter*)ZGetGame()->m_CharacterManager.Find(*uid);
		...
		// Custom add to win/loss count on round end
	case MC_MATCH_SPY_ROUNDRESULT:
	{
		int nTeamWinner;
		pCommand->GetParameter(&nTeamWinner, 0, MPT_INT);
		for (auto itor = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); ++itor)
		{
			ZCharacter* pCharacter = (ZCharacter*)(*itor).second;
			if (pCharacter->GetTeamID() == (MMatchTeam)nTeamWinner)
			{
				pCharacter->GetStatus().Ref().nWinCount += 1;
				pCharacter->GetStatus().Ref().nScore += 10;
				pCharacter->GetStatus().MakeCrc();
			}
			else
			{
				pCharacter->GetStatus().Ref().nLossCount += 1;
				pCharacter->GetStatus().MakeCrc();
			}
		}
	}
	...
	case MC_SPY_GAME_RESULT	:
		{
			...
			for(int i = 0; i < MGetBlobArrayCount(pBlob); i++)
			{
				...
				ZCharacter* pCharacter = (ZCharacter*)ZGetGame()->m_CharacterManager.Find(info->uidPlayer);
				if(!pCharacter) continue;
	...
	case MC_MATCH_GAME_DEAD	:
		{
			...
			ZCharacter* pCharacter = (ZCharacter*)ZGetGame()->m_CharacterManager.Find(uidVictim);
			if(!pCharacter) break;
			...
			if(pCharacter->GetTeamID() == MMT_RED)
			{
				...
				for(ZCharacterManager::iterator i = ZGetGame()->m_CharacterManager.begin(); i != ZGetGame()->m_CharacterManager.end(); i++)
				{
					ZCharacter* pCurrCharacter = (ZCharacter*)(*i).second;

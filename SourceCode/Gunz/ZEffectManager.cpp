...
//	m_pRocketSmokeEffect		= m_pEffectMeshMgr->Get("rocket_smoke_effect");
	...
	m_pCharacterIcons[5] = m_pEffectMeshMgr->Get("ef_pre_all.elu"); // Custom: firstblood icon "ef_pre_firs.elu"
...
#define SMOKE_VELOCITY				110.f
	...
	if(bDrawTargetEffects) {
		...
		if(nTargetType==ZTT_OBJECT){	// 타겟이 맵일때
			...
			if(wtype != MWT_SHOTGUN && ZGetConfiguration()->GetVideo()->bShotEffects) // 샷건의 한알 한알은 연기 생략
			{
				// 타겟 총탄 연기
			...
			// 피 분출
		#define TARGET_BLOOD_MAX_SCALE		05.0f
		#define TARGET_BLOOD_MIN_SCALE		06.0f
		#define TARGET_BLOOD_LIFE_TIME		5
		#define TARGET_BLOOD_VELOCITY		2.0f	// meter/sec

			// Custom: Enabled blood effects
			if (ZGetGame() && ZGetConfiguration()->GetEtc()->bBlood)
			{
				for (int i = 0; i < 3; i++)
				{
					...
					float fSize = 1.0f + float(rand() % 100) / 100.0f;
					AddSmokeEffect(m_pEBSBloods[rand() % BLOOD_COUNT], p, TargetNormal * TARGET_BLOOD_VELOCITY, rvector(0, 0, 0), TARGET_BLOOD_MIN_SCALE * fSize, TARGET_BLOOD_MAX_SCALE * fSize, TARGET_BLOOD_LIFE_TIME);
					Add(pNew);
				}
			}
...
void ZEffectManager::AddBloodEffect(rvector& Target, rvector& TargetNormal)
{
	...
	{
		ZEffect* pNew = NULL;
#define TARGET_BLOOD_MAX_SCALE2		2.0f
#define TARGET_BLOOD_MIN_SCALE2		1.0f
#define TARGET_BLOOD_LIFE_TIME2		5
#define TARGET_BLOOD_VELOCITY		5.0f	// meter/sec
		...
		for (int i = 0; i < 3; i++)
		{
			...
			float fSize = 1.0f + float(rand() % 100) / 100.0f;
			AddSmokeEffect(m_pEBSBloods[rand() % BLOOD_COUNT], p, TargetNormal, rvector(0, 0, 0), TARGET_BLOOD_MIN_SCALE2 * fSize, TARGET_BLOOD_MAX_SCALE2 * fSize, TARGET_BLOOD_LIFE_TIME2);
			Add(pNew);
		}

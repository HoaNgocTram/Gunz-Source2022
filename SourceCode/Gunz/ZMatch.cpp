...
#ifdef _SPEC 1
			else if (ZGetGame()->m_pMyCharacter->GetTeamID() != MMT_SPECTATOR)
			{
...
void ZMatch::OnForcedEntry(ZCharacter* pCharacter)
{
	...
	if (pCharacter == ZGetGame()->m_pMyCharacter)
	{
		...
		if (pObjCache && pObjCache->CheckFlag(MTD_PlayerFlags_AdminHide)) {
			...
			// 팀플레이면 스팩테이터
			if (IsWaitForRoundEnd() || ZGetGameTypeManager()->IsQuestChallengeOnly(GetMatchType()) || ZGetGameTypeManager()->IsQuestDerived(GetMatchType()) || ZGetGameTypeManager()->IsBlitzQuest(GetMatchType()))
			{
				pCharacter->SetVisible(false);
				pCharacter->ForceDie();
	...
	else
	{
		...
		if (IsWaitForRoundEnd() && (GetRoundState() != MMATCH_ROUNDSTATE_FREE) ||
			ZGetGameTypeManager()->IsQuestChallengeOnly(GetMatchType()) || ZGetGameTypeManager()->IsBlitzQuest(GetMatchType()) || ZGetGameTypeManager()->IsQuestDerived(GetMatchType()))
		{
			if (pCharacter != NULL)
			{
				pCharacter->SetVisible(false);
...
//				int nMode = GetMatchType();
//				int nDelayAfterDying = 0;
//				switch (nMode)
//				{
//				case MMATCH_GAMETYPE_BLITZKRIEG:
//					nDelayAfterDying = 15000;
//				//	mlog("Blitz time 15 sec.\n");
//					break;
//				default:
//					nDelayAfterDying = 5000;
//				//	mlog("Normal 5 sec.\n");
//					break;
//				}

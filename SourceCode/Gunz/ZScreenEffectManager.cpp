...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	m_pHPPanelNew = new ZScreenEffect(m_pEffectMeshMgr->Get("panelold")); //Custom: Toggle change bar HP

	m_pHPPanelEffect = new ZScreenEffect(m_pEffectMeshMgr->Get("hppaneleffect"));
	m_pScorePanel	= new ZScreenEffect(m_pEffectMeshMgr->Get(""));
	m_pBuffPanel = new ZScreenEffect(m_pEffectMeshMgr->Get(""));
...
void ZScreenEffectManager::UpdateEffects()
{
	...
	bool isNewQuestDerived = ZGetGameTypeManager()->IsQuestChallengeOnly(ZGetGameClient()->GetMatchStageSetting()->GetGameType());
	bool IsBlitzQuest = ZGetGameTypeManager()->IsBlitzQuest(ZGetGameClient()->GetMatchStageSetting()->GetGameType());

	if (!isNewQuestDerived && !isQuestDerived && !IsBlitzQuest)
		return;

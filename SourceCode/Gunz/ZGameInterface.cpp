...
//	SetListenerWidget("StagePlayerListTabFriend", ZGetListenerGamePlayerListTabFriend());
...
#ifdef _CW_VOTE
	SetListenerWidget("LadderWarsVoteButton0", ZGetLadderWarsVote0());
	SetListenerWidget("LadderWarsVoteButton1", ZGetLadderWarsVote1());
	SetListenerWidget("LadderWarsVoteButton2", ZGetLadderWarsVote2());
#endif // _CW_VOTE
	...
	SetListenerWidget("ChannelList_LadderWars", ZGetChannelList());
...
//	SetListenerWidget("MapList_Caller", ZGetStageMapListCallerListener());
	...
	*/
	...
	SetListenerWidget("BuyEventConfirmCaller", ZGetBuyEventButtonListener()); // Custom: Event Buy Fix Desperate
	...
	SetListenerWidget("EventEquipmentListCaller", ZGetEventEquipmentListCallerButtonListener());
	...
	SetListenerWidget("EventEquipmentList", ZGetEventShopItemListBoxListener());
...
bool ZGameInterface::OnGameCreate(void)
{
	...
	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_BLITZKRIEG)
	{
		...
		{
		case (0):
			strcpy(szFileName, "Interface/default/loading/loading_blue01.png");
			break;
		case (1):
			strcpy(szFileName, "Interface/default/loading/loading_blue02.png");
			break;
		case (2):
			strcpy(szFileName, "Interface/default/loading/loading_red01.png");
			break;
		case (3):
			strcpy(szFileName, "Interface/default/loading/loading_red02.png");
			break;
		}
	}
	...
	*/
	...
	if ((ZApplication::GetInstance()->GetLaunchMode() == ZApplication::ZLAUNCH_MODE_STANDALONE_REPLAY) ||
		(ZGetGameClient()->IsCWChannel()) || ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_MELEEONLY) || // Custom: Exploit change weapons in equipament
	...
	else if ((ZGetGameClient()->IsLadderChannel()))
	{
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_OPTION, true);
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_CLOSE, true);
		m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_BATTLE_EXIT, false);
...
//#endif
	...
	((MBitmapR2*)m_pLoginPanel)->Create("loginpanel.tga", RGetDevice(), "Interface/loadable/loginpanel.tga");
	if (m_pLoginPanel)
	{
		// ÀÐ¾î¿Â ºñÆ®¸Ê ÀÌ¹ÌÁö Æ÷ÀÎÅÍ¸¦ ÇØ´ç À§Á¬¿¡ ³Ñ°ÜÁà¼­ Ç¥½ÃÇÑ´Ù
...
//		pServerList->AddServer( "Billcap", "192.168.0.132", 6000, 2, 0, 1000, true);
	...
	//		pServerList->AddServer( "Debug Server", "127.0.0.1", 6000, 4, 0, 1000, true);			// Debug server
		...
		if (pServerList->GetServerList().size() == 0)
		{
			...
			pServerList->AddServer("Gunz Online (Backup...)", const_cast<char*>(strIP.c_str()), 6000, 3, 0, 1, true, const_cast<char*>(strIP.c_str()), 0, 0, 5100);
		}
#endif
#else
...
void ZGameInterface::BackGroundButtom()
{
	...
	bool bLadderWarsUI = (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_LADDER);
...
//	SetState(m_nInitialState);
	...
	ZGetGameTypeManager()->SetGameTypeStr(MMATCH_GAMETYPE_BLITZKRIEG, ZMsg(MSG_MT_BLITZ));
...
void ZGameInterface::ChangeWeapon(ZChangeWeaponType nType)
{
	...
	if (pChar->IsDie() ) return;						// Á×¾úÀ¸¸E¹«½ÃÇÑ´Ù.	
	if (m_pGame->GetMatch()->IsRuleGladiator()) return;	// ±Û·¡µð¿¡ÀÌÅÍ¸EÄ®¸¸½á¶E


	int nParts = -1;

/*
	if( m_bisReserveChangeWeapon ) { // ¿¹¾àµÈ»óÅÂ¿¡¼­ ¹Ù²Ù·Á°EÇÏ´Â°æ¿E¿¹¾EÃEÒ~
		m_bisReserveChangeWeapon = false;
	}

	if( m_pMyCharacter->m_fNextShotTime > g_pGame->GetTime()) { // ¾ÆÁE¹Ù²Ü¼öÀÖ´Â½Ã°£ÀÌ ¾Æ´Ï µÇ¾úÀ¸´Ï ¿¹¾à~
...
*/
	bool bWheel = false;		// ÈÙ·Î ÀÛµ¿ÇßÀ»¶§´Â Äµ½½µÇÁE¾Ê´Â´Ù
	if ((nType == ZCWT_PREV) || (nType == ZCWT_NEXT))
	{
		...
		for(int i = MMCIP_MELEE; i < MMCIP_CUSTOM3 + 4; i++)
		{
	...
	// ¸Å´Þ·ÁÀÖÀ»¶§/½ò¶§/´ýºúÔµ/½ºÅ³/¸·À»¶§/¶ç¿öÁø»óÅÂ´Â ¹«±â±³È¯ÀÌ ¾ÈµÈ´Ù.
	if (zStatus.m_bWallHang || zStatus.m_bShot || zStatus.m_bShotReturn || uStatus.m_bTumble 
		...
		|| zStatus.m_bSlash || zStatus.m_bJumpSlash || zStatus.m_bJumpSlashLanding	// °­º£±EÁß¿¡µµ ±³È¯ ¾ÈµÈ´Ù
		 
		// ¼¦ÁßÀÏ¶§ »óÃ¼¾Ö´Ï¸ÞÀÌ¼ÇÀ¸·Î ÆÇº°
		|| (pChar->GetStateUpper() == ZC_STATE_UPPER_SHOT && pChar->IsUpperPlayDone() == false )
		
		/*
		// ±×¸®°E¹ß»çÇÏ°úÏª¼­ µô·¹ÀÌ°¡ ¾ÆÁE¾È³¡³µÀ»¶§
	...
	/*
	// »óÃ¼¾Ö´Ï¸ÞÀÌ¼ÇÀÌ ÀÖÀ¸¸EÄµ½½.. ¸®·ÎµE¼¦ µûÀ§
	...
	*/
	...
	// ³» ¹«±â´Â Á÷Á¢ ¹Ù²Û´Ù. command¸¦ ÅEÏ¸E¼ø¼­°¡ ¹Ù²ûÐÂ °æ¿E¡ ÀÖ´Ù.
...
#define SCREENSHOT_DELAY		2000
	...
	sprintf(szFilenameSafe,"nocharname_%4d%02d%02d_%02d%02d%02d",  
		t.wYear, t.wMonth, t.wDay, t.wHour, t.wMinute, t.wSecond);	// Ä³¸¯¸úÜ» »ý·«ÇÑ ¹öÀE
	...
	*/
	...
	//if(nsscount<1000)
	{
		...
		if(frontbuffer->LockRect(&LRECT,&rt,NULL)==D3D_OK)
		{
			...
			char szFullPath[_MAX_PATH];	// È®ÀåÀÚ´Â ¾Èµé¾ûÜÖÀ½
			...
			bool bSuccess = RScreenShot(nWidth,nHeight, data, nWidth*nHeight * 4, szFullPath);

			if(!bSuccess) {	// Ä³¸¯¸úÒ§¹®¿¡ ½ÇÆÐÇßÀ» ¼EÀÖÀ¸´Ï Ä³¸¯¸úÜ» »ý·«ÇÑ ÆÄÀÏ¸úÜ¸·Î Àç½Ãµµ
				...
				bSuccess = RScreenShot(nWidth,nHeight, data, nWidth*nHeight * 4, szFullPath);
...
void ZGameInterface::ShowShopDialog(bool bShow)
{
	...
	{	// »óÁ¡ ¼±ÅÃ
		...
		if (pWidget != NULL)
			pWidget->Enable(false);
		pWidget = pResource->FindWidget("BuyEventConfirmCaller");
		if (pWidget != NULL)
			pWidget->Enable(false);
...
#endif // _CW_VOTE
{
	...
	if (isvote == true)
	{
		...
		pWidget = m_IDLResource.FindWidget("LadderWarsMapVote");
		if (pWidget)
		{
			pWidget->Show(bFinding);
			if (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_CLAN || ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_LADDER)
				pWidget->SetText("Map Voter");
			else
				pWidget->SetText("Map Voter");
		}
	}
...
int MDrawContext::TextMultiLine(MRECT& r, const char* szText, int nLineGap, bool bAutoNextLine, int nIndentation, int nSkipLine, MPOINT* pPositions)
{
	...
	do {
		...
		if (ZGetGame() && ZGetConfiguration()->GetEtc()->bChatBackground)
		{

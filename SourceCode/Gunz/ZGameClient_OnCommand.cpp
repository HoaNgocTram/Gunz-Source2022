...
#ifdef _CW_VOTE //Custom: Interruptor Vote Map
	case MC_MATCH_RANDOM_MAPS:
	{
		int nRandomIndex[3];
		...
		for (int i = 0; i < 3; i++)
		{
			sprintf(Name, "LadderWarsMap%d", i);
...
//				pCommand->GetParameter(szProposerCharName,	1, MPT_STR);
		...
		{
			bool BlitzKrieg;
			pCommand->GetParameter(&BlitzKrieg, 0, MPT_BOOL);
		...
		case MC_MATCH_LADDERWARS_COUNTER:
		{
			if (ZGetGameInterface()->GetState() == GUNZ_LOBBY)
			{
				...
				for (int i = 0; i <= 7; i++)
				{
					...
					sprintf(szOutput, "%d", Info->Count[i]);
		...
		case MC_MATCH_LADDER_LAUNCH:		// 게임 시작
			{
				...
				pCommand->GetParameter(szMapName, 1, MPT_STR, sizeof(szMapName) );
				bool LadderWars;
				pCommand->GetParameter(&LadderWars, 2, MPT_BOOL);
				OnLadderLaunch(uidStage, szMapName, LadderWars);
			}break;
		case MC_MATCH_LADDERWARS_CHARINFO:
		{
				pCommand->GetParameter(&m_LadderWarsCharInfo.Ranking,			0, MPT_INT);
				pCommand->GetParameter(&m_LadderWarsCharInfo.Wins,				1, MPT_INT);
				pCommand->GetParameter(&m_LadderWarsCharInfo.Losses,			2, MPT_INT);
				pCommand->GetParameter(&m_LadderWarsCharInfo.Draws,			3, MPT_INT);
				pCommand->GetParameter(&m_LadderWarsCharInfo.Score,			4, MPT_INT);

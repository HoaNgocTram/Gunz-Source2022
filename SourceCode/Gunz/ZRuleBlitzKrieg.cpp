...
bool ZRuleBlitzKrieg::Init()
{
	...
	if (ZGetNpcMeshMgr()->LoadXmlList("model/npc3.xml") == -1)
	{
		mlog("Error while Load NPC3 Models.\n");
		return false;
	}
	mlog("npc3 model has loaded successfully! \n");
	if (!m_pActorDefMgr->ReadXml(ZApplication::GetFileSystem(), "system/npc3.xml"))
	{
		mlog("Error while Read npc3.\n");
		return false;
	}
	mlog("npc3 has loaded successfully! \n");
	return true;
}
...
void ZRuleBlitzKrieg::OnSetRoundState(MMATCH_ROUNDSTATE roundState)
{
	...
	case MMATCH_ROUNDSTATE_COUNTDOWN:
	{
		...
		m_roundStartTime = 5000; //Custom: Time Blitz

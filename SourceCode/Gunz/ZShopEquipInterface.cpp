...
/*#ifndef _DEBUG
	...
	pButton = (MButton*)pResource->FindWidget("BuyEventConfirmCaller");
	if (pButton)
	{
		pButton->Show(false);
		pButton->Enable(false);
	}// Custom: Event Buy Fix Desperate
	...
	pButton = (MButton*)pResource->FindWidget("EventEquipmentListCaller");
	if (pButton != NULL)
		pButton->Show(nTabIndex != 3 ? true : false);
	...
	pPicture = (MPicture*)pResource->FindWidget("Shop_FrameTabLabel4");
	if (pPicture)
		pPicture->Show(nTabIndex == 3 ? true : false);
	...
	if ( pPicture)
	{
		if ( nTabIndex == 0)
			...
			pBitmap = MBitmapManager::Get("framepaneltab4.tga");
...
#ifndef _DEBUG
#if defined(LOCALE_BRAZIL) || defined(LOCALE_INDIA) || defined(LOCALE_US) || defined(LOCALE_JAPAN) || defined(LOCALE_KOREA) || defined(LOCALE_NHNUSA)
	if (ZGetShop()->GetShopType() == ZSHOP_MEDAL)
	{
		...
		pWidget = pResource->FindWidget("EventEquipmentListCaller");
		if (pWidget)  pWidget->Show(false);

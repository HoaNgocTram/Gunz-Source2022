...
bool ZWorld::Create(ZLoadingProgress* pLoading)
{
	...
	{
		if (!ZGetConfiguration()->GetEtc()->bNewRenderer)
...
//					water_instance->m_nWaterType = 1;//sea
	...
	else if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_VAMPIRE))
	{
		m_bFog = true;
		m_fFogNear = 0.0f;
		m_fFogFar = 6000.0f;
	...
	else if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_SNIPERONLY))
	{
		m_bFog = true;
		m_fFogNear = 20.8f;
		m_fFogFar = 9000.0f;

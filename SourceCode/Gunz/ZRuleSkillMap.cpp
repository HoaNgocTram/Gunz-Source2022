	...
	switch (command->GetID())
	{
		case MC_SKILLMAP_OBTAIN_ITEM:
		{
			...
			if (pCharacter == ZGetGame()->m_pMyCharacter)
			{
				...
				sprintf(szText, "You arrived! You got %d XP and %d BP for getting here! ", xp, bp);
...
void ZRuleSkillMap::OnDraw(MDrawContext* pDC)
{
	...
	ZBmNumLabel* bmNumLabel = (ZBmNumLabel*)ZGetGameInterface()->GetIDLResource()->FindWidget("CountDownTime");
	ZBmNumLabel* bmNumLabelRed = (ZBmNumLabel*)ZGetGameInterface()->GetIDLResource()->FindWidget("CountDownTime_Red");
	...
	{
		char bestTime[128];
		MFont* pFont = FontSkill();
		pDC->SetFont(pFont);
		pDC->SetColor(MCOLOR(0xFFFFFFFF));
		pDC->SetFont(MFontManager::Get("NEWFONT"));
		/*sprintf_s(bestTime, "Record : %02d:%02d:%02d", static_cast<int>(myBestTime / 1000 / 60), static_cast<int>(myBestTime / 1000 % 60), ((int)myBestTime / 10) % 100);
		pDC->Text(350.f / 800.f, 55.f / 600.f, bestTime);*/
	}
}

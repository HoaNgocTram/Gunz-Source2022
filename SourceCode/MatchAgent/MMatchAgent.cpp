...
bool MMatchAgent::OnCommand(MCommand* pCommand)
{
	switch(pCommand->GetID()){
		case MC_LOCAL_LOGIN:		// Local
			{
				...
				{
					LOG(LOG_FILE, "LocalLogin Invalid Sender\n");
					return false;
				}
		...
		case MC_NET_CLEAR:			// Local
			{
				...
				if (pCommand->GetSenderUID() != this->GetUID())
				{
					LOG(LOG_FILE, "NetClear Invalid Sender\n");
					return false;
				}
		...
		case MC_NET_CHECKPING:		// Local
			{
				...
				if (pCommand->GetSenderUID() != this->GetUID())
				{
					LOG(LOG_FILE, "CheckPing Invalid Sender\n");
					return false;
				}
		...
		case MC_AGENT_CONNECT:		// Local
			{
				...
				if (pCommand->GetSenderUID() != this->GetUID())
				{
					LOG(LOG_FILE, "Connect Invalid Sender\n");
					return false;
				}
		...
		case MC_AGENT_DISCONNECT:	// Local
			{
				...
				if (pCommand->GetSenderUID() != this->GetUID())
				{
					LOG(LOG_FILE, "Disconnect Invalid Sender\n");
					return false;
				}
		...
		case MC_AGENT_LOCAL_LOGIN:	// Local
			{
				...
				if (pCommand->GetSenderUID() != this->GetUID())
				{
					LOG(LOG_FILE, "AgentLocal Invalid Sender\n");
					return false;
				}
		...
		case MC_AGENT_TUNNELING_TCP:	// M2M
			{
				// Custom: Proxy patch (TCP is not used)
				//break;
...
bool MMatchAgent::UDPSocketRecvEvent(DWORD dwIP, WORD wRawPort, char* pPacket, DWORD dwSize)
{
	...
	if ((dwSize < pPacketHeader->nSize) || ((pPacketHeader->nMsg != MSGID_COMMAND) && (pPacketHeader->nMsg != MSGID_RAWCOMMAND)) ) 
		return false;
	...
	case MSGID_RAWCOMMAND:
		{
			...
			if (pCmd->GetID() == MC_UDP_PING)
			{
                ...
                char szIP[64]; strcpy(szIP, inet_ntoa(addr.sin_addr));
                unsigned int nPort = ntohs(addr.sin_port);
                
                MUID uidSender = FindClientUID(szIP, nPort);
                if(uidSender == MUID(0, 0)) break;
                
                MCommandParameterUID* pParamSenderUID = (MCommandParameterUID*)pCmd->GetParameter(0);
                if(pParamSenderUID == NULL || pParamSenderUID->GetType() != MPT_UID) break;
                
                char nData[1024];
                MCommandParameterUID(uidSender).GetData(nData, sizeof(nData));
			...
			{
				// Custom: Free memory for unused command.
				delete pCmd;
			} 
		}
		break;
...
// Custom: Proxy patch
/*
MUID MMatchAgent::FindClientUID(const char* pszIP, WORD wPort)
{
    for(MAgentClients::iterator i = m_Clients.begin(); i != m_Clients.end(); i++)
    {
        MAgentClient* pClient = (*i).second;
        
        if(stricmp(pClient->GetIP(), pszIP) == 0 && pClient->GetPort() == wPort)
        {
            return pClient->GetUID();
        }
    }

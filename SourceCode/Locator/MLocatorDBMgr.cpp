	...
	if( CheckOpen() && (0 != pServerStatusMgr) )
	{
		...
		try
		{
			if (GetLocatorConfig()->IsTestServerOnly() == true) {
				...
				rs.Open("SELECT ServerID, Type, CurrPlayer, MaxPlayer, CONVERT(varchar(20), Time, 20) AS Time, IP, Port, ServerName, Opened, AgentIP, PeerPort FROM ServerStatus(NOLOCK) WHERE (Opened <> 0) AND (Opened IS NOT NULL) AND Type=6", 
		...
		if( rs.IsOpen() )
		{
			...
			for( ; !rs.IsEOF(); rs.MoveNext() )
			{
				...
				ss.SetPeerPort( rs.Field( "PeerPort" ).AsInt() );
#endif

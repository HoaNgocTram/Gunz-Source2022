...
bool MLocator::Create()
{
	...
	// Custom: Updated strings to English
	if( !InitDBMgr() )
	{
		mlog( "MLocator::Create - DB initialization failed.\n" );
		return false;
	}

	if( !InitServerStatusMgr() )
	{
		mlog( "MLocator::Create - ServerStatusMgr member initialization failed.\n" );
		return false;
	}

	if( !InitUDPManager() )
	{
		mlog( "MLocator::Create - UDP Manager member initialization failed.\n" );
		return false;
	}

	if( !InitCountryCodeFilter() )
	{
		mlog( "MLocator::Create - Failed to initialize the country code filter.\n" );
		return false;
	}

	if( !InitSafeUDP() )
	{
		mlog( "MLocator::Create - SafeUDP initialization failed.\n" );
		return false;
	}
			...
			if( GetLocatorDBMgr()->GetServerStatus(m_pServerStatusMgr) )
			{
				...
				if( 0 != m_vpServerStatusInfoBlob )
				{
					...
					for( int i = 0; i < m_nLastGetServerStatusCount; ++i )
					{
						...
						pMTDss->m_nPeerPort		= (*m_pServerStatusMgr)[i].GetPeerPort();
#endif

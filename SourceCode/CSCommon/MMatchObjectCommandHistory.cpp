...
#ifdef _NOLEAD
			pHistory->m_commands.size() > MAX_COMMAND_HISTORY_COUNT && nCommandID != MC_PEER_NOLEAD && nCommandID != MC_PEER_NOLEADBLOB && nCommandID != MC_MATCH_BRIDGEPEER)
			*pbFloodingSuspect = true;
#else
			pHistory->m_commands.size() > MAX_COMMAND_HISTORY_COUNT && nCommandID != MC_MATCH_BRIDGEPEER )
			*pbFloodingSuspect = true;
#endif
		else

...
{
public:
	MQuestChallengePlayerLogInfo() : m_nCID(0)
	{
	}

	~MQuestChallengePlayerLogInfo()
	{}
...
class MMatchQuestChallengeGameLogInfoManager : public map< MUID, MQuestChallengePlayerLogInfo* >
{
private:
	MQuestChallengePlayerLogInfo* Find(const MUID& uidPlayer);
public:
	MMatchQuestChallengeGameLogInfoManager();
	virtual ~MMatchQuestChallengeGameLogInfoManager();
	...
	void SetStartTime(const DWORD dwStartTime) { m_dwStartTime = dwStartTime; }
	void SetEndTime(const DWORD dwEndTime) { m_dwEndTime = dwEndTime; }
	void SetScenarioID(const char* pszScenarioID);

	bool PostInsertQuestChallengeGameLog();
private:
	int		m_nMasterCID;
	char	m_szScenarioID[STAGENAME_LENGTH];
	DWORD	m_dwStartTime;
	DWORD	m_dwEndTime;
	char	m_szStageName[STAGENAME_LENGTH];
};

...
void MAddSharedCommandTable(MCommandManager* pCommandManager, int nSharedType)
{
	...
	if(nSharedType&MSCT_MATCHSERVER || nSharedType&MSCT_CLIENT){
		...
		C(MC_MATCH_RESPONSE_LOGIN, "Match.ResponseLogin", "Response Login", MCDT_MACHINE2MACHINE)
			...
			P(MPT_INT, "Country") // Custom: Fix no login
			P(MPT_INT, "Cash")
			P(MPT_INT, "Event")
			P(MPT_UID,	"uidPlayer")
			P(MPT_BOOL,	"bEnabledSurvivalMode")
			P(MPT_BOOL,	"bEnabledDuelTournament")
			P(MPT_BLOB,	"EncryptMsg")		// HShieldø°º≠ ªÁøÅEœ¥¬ æœ»£∏ﬁºº¡ÅE

			/* ServerKeeperø°º≠µµ ªÁøÅEª «œ±‚¿ß«ÿº≠ ø‹∫Œ∑Œ ø≈±ÅE - by √ﬂ±≥º∫.
		...
		C(MC_MATCH_JOIN_LADDERWARS, "knaw65goag", "jnaaw", MCDT_MACHINE2MACHINE);
			P(MPT_INT, "Arg1")
			P(MPT_INT, "Arg2")
...
//			P(MPT_BLOB, "SKey")
		...
		C(MC_MATCH_LADDER_REQUEST_CHALLENGE, "Match.Ladder.Request.Challenge", "Request Challenge a Ladder", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "MemberCount")		// ∑π¥ı∆¿ ∏‚πÅEºÅE
			P(MPT_UINT, "Options")			// ø…º«
			P(MPT_BLOB, "MemberNames")		// ∑π¥ı∆¿ ∏‚πÅE¿Ã∏ßµÅE
			P(MPT_UINT, "GameType")
...
//			P(MPT_VECTOR, "Position")
		...
		C(MC_NEWQUEST_NPC_SPAWN,				"NewQuest.NPC.Spawn",	"NPC Spawn", MCDT_MACHINE2MACHINE)
			...
			P(MPT_UID, "uidNpc")
			P(MPT_STR, "szNpcDefName")	//todok ∆–≈∂ ¿˝æ‡¿ª ¿ß«ÿ πÆ¿⁄ø≠∏ª∞Ì enum∞™¿ª ≥—±Ê ºˆ ¿÷∞‘ «œ¿⁄
			P(MPT_UCHAR, "nSpawnType")			// NPC Ω∫∆˘ ¡æ∑˘ ¿Œµ¶Ω∫
			P(MPT_UCHAR, "PositionIndex")		// NPC Ω∫∆˘ ¿ßƒ° ¿Œµ¶Ω∫
...
//	CA("connect", "Net.Connect");
	...
	C(MC_MATCH_LADDERWARS_CHARINFO, "MC_MATCH_BLITZKRIEGS_CHARINFO", "Player Wars Character Info", MCDT_MACHINE2MACHINE)
		P(MPT_INT, "Ranking")
		P(MPT_INT, "Wins")
		P(MPT_INT, "Loses")
		P(MPT_INT, "Draws")
...
#ifdef _ANTILEADNEW
	C(MC_GUNZ_ANTI_LEAD_JORK, "Match.Gunz.AntiLead", "AntiLead Information", MCDT_PEER2PEER);
	P(MPT_BLOB, "AntiLeadJork");
#endif
...
#ifdef _CW_VOTE
	C(MC_MATCH_RANDOM_MAPS, "Maps", "Maps", MCDT_MACHINE2MACHINE)
		P(MPT_INT, "1");
	    P(MPT_INT, "2");
	    P(MPT_INT, "3");
	C(MC_MATCH_VOTE, "Maps", "Maps", MCDT_MACHINE2MACHINE)
		P(MPT_INT, "Map");
	C(MC_MATCH_VOTE_UPDATE, "Maps", "Maps", MCDT_MACHINE2MACHINE)
		P(MPT_INT, "1");
	    P(MPT_INT, "2");
	    P(MPT_INT, "3");
	...
	C(MC_MATCH_LADDERWARS_FRIENDACCEPT, "AcceptFriend", "AcceptFriend", MCDT_MACHINE2MACHINE)
		C(MC_MATCH_LADDERWARS_CHARINFO, "LeaveFriend", "LeaveFriend", MCDT_MACHINE2MACHINE)

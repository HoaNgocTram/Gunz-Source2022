...
*/
	...
	if( !pDBMgr->InsertGameLog(m_nMasterCID, m_szMap, m_szGameType, m_szGameName, &m_nID) )
	{
		SetResult(MASYNC_RESULT_FAILED);
		return;
	}
...
{// Custom: Support for more than 4 people in quest.
/*
	//if( MSM_TEST == MGetServerConfig()->GetServerMode() ) 
	//{
...
MAsyncDBJob_InsertQuestChallengeGameLog::~MAsyncDBJob_InsertQuestChallengeGameLog()
{
	vector< MQuestChallengePlayerLogInfo* >::iterator it, end;
	it = m_Player.begin();
	end = m_Player.end();
	for (; it != end; ++it)
		delete (*it);
...
bool MAsyncDBJob_InsertQuestChallengeGameLog::Input(const char* pszStageName,
	const char* szScenarioName,
	const int nMasterCID,
	MMatchQuestChallengeGameLogInfoManager* pQGameLogInfoMgr,
	DWORD dwElapsedPlayerTime)
{
	if (0 == pQGameLogInfoMgr)
		return false;
	...
	m_szScenarioName = szScenarioName;
	m_nMasterCID = nMasterCID;
	m_dwElapsedPlayTime = dwElapsedPlayerTime;

	int										i;
	MQuestChallengePlayerLogInfo* pPlayer;
	MMatchQuestChallengeGameLogInfoManager::iterator itPlayer, endPlayer;
	...
	for (; itPlayer != endPlayer; ++itPlayer)
	{
		...
		if (0 == (pPlayer = new MQuestChallengePlayerLogInfo))
		{
			continue;
		}
...
void MAsyncDBJob_InsertQuestChallengeGameLog::Run(void* pContext)
{
	//if( MSM_TEST == MGetServerConfig()->GetServerMode() ) 
	//{
	MMatchDBMgr* pDBMgr = reinterpret_cast<MMatchDBMgr*>(pContext);
	...
	// 우선 퀘스트 게임 로그를 저장함.
	if (!pDBMgr->InsertQuestChallengeGameLog(m_szStageName,
		m_szScenarioName,
		m_nMasterCID, m_PlayersCID[0], m_PlayersCID[1], m_PlayersCID[2],
		m_dwElapsedPlayTime))
	{
	...
	int											i;
	int											nCID;
	vector< MQuestChallengePlayerLogInfo* >::iterator	itPlayer, endPlayer;

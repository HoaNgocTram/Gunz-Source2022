...
bool MMatchDBMgr::GetLadderWarsCharInfo(DWORD dwCID, LadderWarsCharInfo* pBlitzCharInfo)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	pBlitzCharInfo->Ranking = rs.Field("Ranking").AsInt();
	pBlitzCharInfo->Wins = rs.Field("Wins").AsInt();
	pBlitzCharInfo->Losses = rs.Field("Losses").AsInt();
	pBlitzCharInfo->Draws = rs.Field("Draws").AsInt();
	pBlitzCharInfo->Score = rs.Field("Score").AsInt();
	_STATUS_DB_END(51);

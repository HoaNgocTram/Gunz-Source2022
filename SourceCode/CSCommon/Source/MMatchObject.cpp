...
void MMatchObject::OnEnterBattle()
{
	...
	if (pChannel && pChannel->GetChannelType() != MCHANNEL_TYPE_LADDER && pChannel && pChannel->GetChannelType() != MCHANNEL_TYPE_CLAN)
	ResetGamePlayInfo();
}
...
void MMatchObject::SetLadderWarsCharInfo(LadderWarsCharInfo* pLadderCharInfo)
{
	// 이미 있는 정보면 어쩌죠? -.,-;
	...
	memcpy(m_pLadderWarsCharInfo, pLadderCharInfo, sizeof(LadderWarsCharInfo));;
}

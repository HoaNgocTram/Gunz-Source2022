...
#include "Blitzkriegs/MMatchRuleBlitzKrieg.h"
...
void MMatchServer::OnStageCreate(const MUID& uidChar, char* pszStageName, bool bPrivate, char* pszStagePassword)
{
	...
	if ((MGetServerConfig()->GetServerMode() == MSM_CLAN) && (pChannel->GetChannelType() == MCHANNEL_TYPE_CLAN)
		&& (pChannel->GetChannelType() == MCHANNEL_TYPE_DUELTOURNAMENT || pChannel->GetChannelType() == MCHANNEL_TYPE_LADDER)) {
		return;
	}
...
//		pObj->SetInvalidStageSettingDisconnectWaitInfo();
	...
	if ((pChannel) && (bCheckChannelRule))
	{
		...
		if (pRule)
		{
			...
			if (pNode->nGameType == MMATCH_GAMETYPE_TURBO)
				bSWRMode = true;

#ifdef _CUSTOMRULEMAPS
			if (!pRule->CheckSWRMap(pNode->nMapIndex) && bSWRMode)
			{
				strcpy(pNode->szMapName, MGetMapDescMgr()->GetMapName(MMATCH_MAP_BLITZKRIEG));
				pNode->nMapIndex = 26;
			}
#endif
	...
	if (MGetGameTypeMgr()->IsQuestDerived(pNode->nGameType) || pNode->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE || pNode->nGameType == MMATCH_GAMETYPE_BLITZKRIEG || pNode->nGameType == MMATCH_GAMETYPE_SKILLMAP)
	{
	...
	// Custom: removed late join setting on quest.
	if (MGetGameTypeMgr()->IsQuestDerived(pNode->nGameType) || pNode->nGameType == MMATCH_GAMETYPE_QUEST_CHALLENGE || pNode->nGameType == MMATCH_GAMETYPE_BLITZKRIEG)
	{
	...
	if (nLastGameType != pSetting->GetGameType())
	{
		...
		/*else if (!bLast_BlitzKrieg && bCurr_BlitzKrieg) //Force Map Blitzkriegs
		{
...
void MMatchServer::OnAdminSBTest(const MUID& uidComm, char* pszTargetName)
{
	...
	// Event Team support
	if (IsAdminGrade(pObj) == false && pObj->GetAccountInfo()->m_nUGrade != MMUG_EVENTTEAM)
		return;

	// Event team can also use this command
	//MMatchChannel* pChannel = FindChannel(pObj->GetChannelUID());
...
void MMatchServer::OnRequestForcedEntry(const MUID& uidStage, const MUID& uidChar)
{
	...
	if (MGetGameTypeMgr()->IsQuestDerived(pStage->GetStageSetting()->GetGameType()) || MGetGameTypeMgr()->IsQuestChallengeOnly(pStage->GetStageSetting()->GetGameType()) || MGetGameTypeMgr()->IsBlitzQuest(pStage->GetStageSetting()->GetGameType()))
	{
		if (pStage->m_pRule)
		{

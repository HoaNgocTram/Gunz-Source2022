...
#include "Ladder/MLadderWarsMgr.h"
#include "BlitzKriegs/Blitz.h"
#ifdef _UPREWARDLEVEL
#include "MMatchLevelUpReward.h"
#endif
#include <Include/IMatchRuleNewQuest.h>
...
#define FILENAME_BLITZKRIEG			    "NewQuest/blitzkrieg.xml"
...
bool MMatchServer::LoadInitFile()
{
	...
	/*if (!MGetBlitz()->ReadXml(FILENAME_BLITZKRIEG))
	{
...
*/
	...
	// MatchServer Challenge Quest
	MMatchRuleQuestChallenge::InitActorDefMgr();
	MMatchRuleQuestChallenge::InitScenarioMgr();
	//MMatchRuleQuestChallenge::GetScenarioMgr();

	// MatchServer BlitzKriegs
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		if (!strcmp(szTagName, MTOK_CHANNEL))
		{
			...
			if( bIsDuelTournament )
				...
				nChannelType = MCHANNEL_TYPE_LADDER;
...
int MMatchServer::ObjectRemove(const MUID& uid, MMatchObjectList::iterator* pNextItor)
{
	...
	// ¿Ã¿ÅE° BlockType¿ª º≥¡§«ﬂ¥Ÿ∏ÅEDB update flag∞° º≥¡§µ»¥Ÿ.
	if( pObj->GetDisconnStatusInfo().IsUpdateDB() ) 
	{
		...
		// æ˜µ•¿Ã∆Æ«œ∏ÅE¥ŸΩ√ DB update flag∏¶ π´»ø»≠ «‘.
	...
	/* ∫Ò¡§ªÛ¿˚¿∏∑Œ ¡◊¥¬ ∫Œ∫–¿Ã∂ÅEµπˆ±ÅEª ¿ß«ÿº≠ ≥™¥≤≥˘¿Ω.
	 CharInfo∞° ª˝º∫µ«±ÅE¿ÅE° ¡◊¥¬ ∞Õ¿Œ¡ÅE»Æ¿Œ«œ¥¬∞‘ ∏Ò¿ÅE
	*/
	// º“∏ÅE⁄ø°º≠ »£√‚¿∫ «œ¡ˆ∏∏ ∏’¿ÅE∏˙Ÿ√¿˚¿∏∑Œ «ÿ¡÷¿⁄.
	if( NULL != pObj->GetCharInfo() )
	{
		pObj->FreeCharInfo();
...
void MMatchServer::OnUserWhisper(const MUID& uidComm, char* pszSenderName, char* pszTargetName, char* pszMessage)
{
	...
	if (!IsAdminGrade(pObj) && pObj->GetAccountInfo()->m_nUGrade != MMUG_EVENTTEAM && pTargetObj->CheckUserOption(MBITFLAG_USEROPTION_REJECT_WHISPER) == true) {
...
void MMatchServer::OnUserWhere(const MUID& uidComm, char* pszTargetName)
{
	...
	if ((IsAdminGrade(pObj) == false) && (IsAdminGrade(pTargetObj) == true) && pTargetObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) ||
		pObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM && pTargetObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) && pTargetObj->GetTeam() == MMT_SPECTATOR)
#else
	if ((IsAdminGrade(pObj) == false) && (IsAdminGrade(pTargetObj) == true) && pTargetObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) ||
		pObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM && pTargetObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide))
#endif
	{
		NotifyMessage( pObj->GetUID(), MATCHNOTIFY_GENERAL_USER_NOTFOUND );
		return;
...
#ifdef _SPEC 1
	if (IsAdminGrade(pObj) && IsAdminGrade(pTargetObj) || pObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM &&
		pTargetObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM && pTargetObj->GetTeam() == MMT_SPECTATOR)
#else
	if (IsAdminGrade(pObj) && IsAdminGrade(pTargetObj) || pObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM &&
		pTargetObj->GetAccountInfo()->m_nUGrade == MMUG_EVENTTEAM)
#endif
	{
...
void MMatchServer::OnAdminNAT(const MUID& uidComm)
{
	...
	if (!IsAdminGrade(pObj) && pObj->GetAccountInfo()->m_nUGrade != MMUG_JORK) return;
...
{
	// ƒ≥∏Ø≈Õ ¿Ã∏ß¿« √÷º“-√÷¥ÅE±Ê¿Ã∏¶ ∞ÀªÁ«—¥Ÿ.
	int nNameLen = (int)strlen( szCharName);
	if ( nNameLen < nMinLength)
		return MERR_TOO_SHORT_NAME;				// ƒ≥∏Ø≈Õ ¿Ã∏ß¿Ã ≥— ¬™¥Ÿ

	if ( nNameLen > nMaxLength)
		return MERR_TOO_LONG_NAME;				// ƒ≥∏Ø≈Õ ¿Ã∏ß¿Ã ≥— ±Ê¥Ÿ
	


	// ∆Ø¡§ ±π∞°ø°º≠ ªÁøÅEœ¥¬ ∆ØºÅE±‚»£ ¡ﬂø° ∞¯πÈ¿∏∑Œ «•Ω√µ… ∏∏«— πÆ¿⁄∏¶ « ≈Õ∏µ«—¥Ÿ.

#ifdef LOCALE_KOREA
	// «—±πæ˚€°º≠¥¬ 2πŸ¿Ã∆Æ πÆ¿⁄¡ﬂø° 0xC9xx ∂«¥¬ 0xFExx , 0xA1A1, 0xA4D4¿Œ ±€¿⁄¥¬ ¿Ã∏ßø° ≥÷¿ª ºÅEæ¯¥Ÿ
	int nCur = 0;
